...
instance Quantum a op tag => HasMeaning (CreateBellPairArgs op tag) a where
    meaning = tryCreateBellPairFrom

instance TestsOrderedLayeredQuantum a test op tag
        => HasMeaning (NonEmpty (Atomic (CreateBellPairArgs op) test tag)) a where
...
instance {-# OVERLAPPABLE #-} (TestsOrderedQuantum a test op tag) 
        => HasMeaning (Atomic (CreateBellPairArgs op) test tag) a where
    meaning (AAction a) = tryCreateBellPairFrom a
    meaning (ATest t)   = test t

instance (TestsOrderedLayeredQuantum a test op tag) => HasMeaning (Atomic (CreateBellPairArgs op) test tag) (Layer a) where
    meaning (AAction a) = orderedTryCreateBellPairFrom a
    meaning (ATest t)   = orderedTest t

instance (HasMeaning (CreateBellPairArgs op at) a, Semigroup a, ParallelSemigroup a, ChoiceSemigroup a) 
  => HasMeaning (OneRoundPolicy (CreateBellPairArgs op at)) a where
...
instance (ChoiceSemigroup a, Monoid a, TestsOrderedLayeredQuantum a test op tag) 
  => HasMeaning (FullPolicy (NonEmpty (Atomic (CreateBellPairArgs op) test tag))) a where
...
instance (MonoidStar a, OrderedSemigroup a, TestsOrderedLayeredQuantum a test op tag) 
  => HasMeaning (StarPolicy (NonEmpty (Atomic (CreateBellPairArgs op) test tag))) a where
...
instance (MonoidStar a, OrderedSemigroup a, Quantum a op tag) 
  => HasMeaning (OrderedStarPolicy (CreateBellPairArgs op tag)) a where
...
instance (MonoidStar a, OrderedSemigroup a, TestsOrderedQuantum a test op tag) 
  => HasMeaning (OrderedStarPolicy (Atomic (CreateBellPairArgs op) test tag)) a where
    meaning (OSPAtomic ta) = meaning ta
    meaning (OSPOrdered p q) = meaning p <.> meaning q
    meaning (OSPSequence p q) = meaning p <> meaning q
    meaning OSPOne = mempty
